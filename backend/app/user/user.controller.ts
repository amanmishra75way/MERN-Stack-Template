...
import { createUserTokens, decodeToken, isValidPassword, verifyToken } from "../common/services/passport-jwt.service";
...
export const changePassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (user.provider === ProviderType.MANUAL) {
    const validPassword = await isValidPassword(currentPassword, user.password!);
    if (!validPassword) {
      throw createHttpError(400, {
...
export const requestResetPassword = asyncHandler(async (req: Request, res: Response) => {
  const { email } = req.body;
  ...
  await sendEmail({
    ...
    html: resetPasswordEmailTemplate(tokens.refreshToken),
  });
